...
int z (struct diretorio * diretorio, struct diretorio * novo_diretorio, char * file_name, FILE * archive_pt, FILE * novo_archive_pt) {
    ...
    insere_s_arquivo(novo_diretorio, file_s, -1);

    // Move todos os membros sizeof(struct arquivo) para frente - Offset atualizado
    move_sequencial(novo_diretorio, novo_archive_pt, 0, sizeof(struct arquivo), -1);

    // Escreve o conteudo do membro no novo archive
    fseek(novo_archive_pt, 0, SEEK_END);
    fwrite(buffer, tam_mem, 1, novo_archive_pt);
    free(buffer);

    // Escreve no archiver a struct diretorio
    escreve_s_diretorio(novo_diretorio, novo_archive_pt);    
